...
class DeltaELoss(nn.Module):
    ...
    def forward(
        self,
        pred: torch.Tensor,   # (B, 3, H, W) sRGB [0,1]
        tgt:  torch.Tensor,   # (B, 3, H, W) sRGB [0,1]
    ) -> torch.Tensor:
    ...
    def _delta_e_2000(
        ...
        delta_e = (term_L + term_C + term_H + term_R + eps).sqrt()
...
class PerceptualLoss(nn.Module):
    ...
    def forward(
        ...
        pred:      torch.Tensor,              # (B, 3, H, W) sRGB [0,1]
        tgt:       torch.Tensor,              # (B, 3, H, W) sRGB [0,1]
        F_sem_pred: Optional[torch.Tensor] = None,  # (B, N, 384) pre-calcolato
        F_sem_tgt:  Optional[torch.Tensor] = None,  # (B, N, 384) pre-calcolato
    ) -> torch.Tensor:
        """
        Se F_sem_pred e F_sem_tgt sono forniti, li riusa direttamente
        (zero costo aggiuntivo). Altrimenti esegue DINOv2 su entrambi.
        """
        if F_sem_pred is not None and F_sem_tgt is not None:
            return F.mse_loss(F_sem_pred, F_sem_tgt)
        ...
        with torch.no_grad():
            out_pred = self.backbone.forward_features(pred)
            out_tgt  = self.backbone.forward_features(tgt)
